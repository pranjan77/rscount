...
rscountClient <- function(url, token = NULL, flatten = TRUE, 
    ...
    ret_client_object_ref[['count_contigs']] <- function(workspace_name, contigset_id) {
        ret <- ret_client_object_ref[['json_rpc_call']]("rscount.count_contigs", list(
    ...
    class(ret_client_object_ref) <- 'rscountClient'
