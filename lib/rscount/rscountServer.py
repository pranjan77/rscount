...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'rscount'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from rscount.rscountImpl import rscount
impl_rscount = rscount(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['rscount.count_contigs_async'] = ['rscount', 'count_contigs']
async_check_methods['rscount.count_contigs_check'] = ['rscount', 'count_contigs']
sync_methods['rscount.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'rscount'
        ...
        self.rpc_service.add(impl_rscount.count_contigs,
                             name='rscount.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['rscount.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "rscount but no authentication header was passed"
                            raise err
